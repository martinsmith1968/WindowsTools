...
namespace NRA.Util.CommandLine
{
    #region Enumerations
    public enum SyntaxDisplayType
    {
        BuiltIn,
        Advanced
    }
    ...
    public enum ArgumentType
    {
        ...
        /// <summary>
        /// Indicates that if this argument is set, parsing will not reporting missing mandatory arguments.
        /// </summary>
        BypassMandatory = 0x08,
    ...
    public sealed class Parser
    {
        ...
        private class Argument
        {
            ...
            {
                this.groupName = attribute.GroupName;
            ...
            private bool ParseValue(Type type, string stringData, out object value)
            {
                ...
                if ((stringData != null || type == typeof(bool)) && (stringData == null || stringData.Length > 0))
                {
                    try
                    {
                        ...
                        {
                            if (stringData.CompareTo("NOW") == 0)
                                value = DateTime.Now;
                            else if (stringData.CompareTo("MIN") == 0)
                                value = DateTime.MinValue;
                            else if (stringData.CompareTo("MAX") == 0)
                                value = DateTime.MaxValue;
                            else
                                value = DateTime.Parse(stringData);
                            return true;
                        }
            ...
            {
                if (value is string || value is DateTime || value is int || value is uint || value is long || value is ulong || value.GetType().IsEnum)
                {
            ...
            public string SyntaxHelp
            {
                get
                {
                    ...
                    else
                    {
                        ...
                        {
                            builder.Append(":<date|now|min|max>");
                        }
                        else
                        {
        ...
        {
            return ParseArguments(arguments, destination, true);
        }
        public static bool ParseArguments(string[] arguments, object destination, bool showErrors)
        {
            ErrorReporter reporter = null;
            if (showErrors)
                reporter = new ErrorReporter(Console.Error.WriteLine);
            else
                reporter = new ErrorReporter(NullErrorReporter);

            return Parser.ParseArguments(arguments, destination, reporter);
        }
        ...
        {
            return ArgumentsUsage(argumentType, columns, 0);
        }

        /// <summary>
        /// Argumentses the usage.
        /// </summary>
        /// <param name="argumentType">Type of the argument.</param>
        /// <param name="columns">The columns.</param>
        /// <param name="minDescriptionSize">Size of the min description.</param>
        /// <returns></returns>
        public static string ArgumentsUsage(Type argumentType, int columns, int minDescriptionSize)
        {
            return (new Parser(argumentType, null)).GetUsageString(columns, minDescriptionSize);
        }
        ...
        private const int spaceBeforeParam = 3;
        private const int indentBeforeSyntax = 1;
        ...
        private struct ArgumentHelpStrings
        {
            ...
            public ArgumentHelpStrings(string syntax, string help, string group)
            {
                this.group = group;
                this.syntax = syntax;
                this.help = help;
        ...
        /// </summary>
        public string GetUsageString(int screenWidth, int requiredHelpDescriptionColumns)
        {
            SyntaxDisplayType displayType = SyntaxDisplayType.BuiltIn;
            displayType = SyntaxDisplayType.Advanced;
            ...
            const int minimumHelpTextColumn = 10;
            ...
            int helpDescColumns = screenWidth - helpTextColumn;
            if (requiredHelpDescriptionColumns > 0 && helpDescColumns < requiredHelpDescriptionColumns)
                helpTextColumn = screenWidth - requiredHelpDescriptionColumns;

            StringBuilder builder = new StringBuilder();
            // Build Program name
            builder.Append(Environment.NewLine);
            if (displayType == SyntaxDisplayType.Advanced)
            {
                ArgumentHelpStrings responseHelp = strings[strings.Length - 1];

                string defaultArg = string.Empty;
                string parmArg = string.Empty;
                string responseArgSep = string.Empty;

                if (this.defaultArgument != null)
                {
                    defaultArg = string.Format(" <{0}>", this.defaultArgument.LongName);
                    responseArgSep = " | ";
                }
                ...
                builder.Append(string.Format("Usage: {0}{1}{2}{3}{4}", GetProgramName(), defaultArg, parmArg, responseArgSep, responseHelp.syntax));
            }
            else
            {
                builder.Append(string.Format("Usage: {0}", GetProgramName()));
            }
            builder.AppendLine();
            ...
            {
                // Group Changed ??
                if (helpStrings.group != currentGroup)
                {
                    builder.AppendLine();
                    if (helpStrings.group != null && helpStrings.group.Length > 0)
                        builder.AppendLine(string.Format("{0}:", helpStrings.group));
                    currentGroup = helpStrings.group;
                }

                // add syntax string
                int syntaxLength = helpStrings.syntax.Length + indentBeforeSyntax;
                builder.Append("".PadRight(indentBeforeSyntax, ' ') + helpStrings.syntax);
        ...
        private static string GetProgramName()
        {
            ...
            if (asm != null)
            {
                programName = Path.GetFileName(asm.Location);
            }
        ...
        private ArgumentHelpStrings[] GetAllHelpStrings()
        {
            ...
            strings[index++] = new ArgumentHelpStrings("@<file>", "Read response file for more options", "Miscellaneous");
        ...
        {
            return new ArgumentHelpStrings(arg.SyntaxHelp, arg.FullHelpText, arg.GroupName);
        }
